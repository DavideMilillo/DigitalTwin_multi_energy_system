...
async def main():
    ...
    print("[Simulation] Feeding offline mock OpenADR event to EMS...")
    # Call handle_event directly
    response = await ems_node.handle_event(mock_event)
    print(f"[Simulation] EMS returned: {response}")

    # Wait for the OpenADR event to be processed (in this offline script it's instant, but just in case)
