...
1. **Multiple OpenADR Events Overwrite Dispatch Result:** The `EMSOpenADRNode` currently stores the dispatch result in a single `self.dispatch_result` variable. If multiple events are received (like the two events in `main_simulation.py`), the later event overwrites the first one. A list or dictionary should be used to manage multiple simultaneous or future events, and the simulation loop should iterate through them.
2. **Infeasible Shed Targets not Handled Properly:** When a shed request is larger than the baseline power, the EMS correctly finds it infeasible but still tries to meet it and then outputs negative or incorrect values in the simulation. The simulation loop should check if `target_limit < 0` or gracefully handle unattainable shed targets without failing the power balance or simulation checks.
3. **Unclosed Client Session:** `aiohttp` session from `OpenADRClient` is not closed upon exiting `main_simulation.py`. We need to explicitly call `await ems_node.client.stop()` or properly close the aiohttp session in an `__aexit__` or cleanup block.
4. **Missing VEN Response Callback in VTN:** The VTN logs a warning: "You did not provide a 'callback', which means you won't know if the VEN will opt in or opt out of your event." We should add an `on_update_event` callback to the `OpenADRServer` in `main_simulation.py` to handle `optIn`/`optOut` responses.
